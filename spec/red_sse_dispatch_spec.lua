...
describe("sse.classify_event — testing events [RED]", function()
  ...
  it("classifies LiveTestingEnabled", function()
    local result = sse.classify_event({ type = "LiveTestingEnabled", data = "{}" })
    assert.are.equal("live_testing_enabled", result.action)
  end)

  it("classifies LiveTestingDisabled", function()
    local result = sse.classify_event({ type = "LiveTestingDisabled", data = "{}" })
...
describe("testing.handle_live_testing_enabled/disabled [RED]", function()
  it("enables live testing from enabled event", function()
    assert.is_function(testing.handle_live_testing_enabled)
    local state = testing.new()
    state = testing.handle_live_testing_enabled(state)
    assert.is_true(state.enabled)
  end)

  it("disables live testing from disabled event", function()
    assert.is_function(testing.handle_live_testing_disabled)
    local state = testing.new()
    state = testing.handle_live_testing_enabled(state)
    assert.is_true(state.enabled)
    state = testing.handle_live_testing_disabled(state)
