...
-- ─── metadata: duration_ms propagation ──────────────────────────────────────

describe("model metadata", function()
  it("stores duration_ms when metadata provided", function()
    local m = model.new()
    m = model.set_cell_state(m, 1, "running")
    m = model.set_cell_state(m, 1, "success", "val x = 42", { duration_ms = 150 })
    local cell = model.get_cell_state(m, 1)
    assert.are.equal("success", cell.status)
    assert.are.equal(150, cell.duration_ms)
  end)

  it("duration_ms is nil when metadata omitted", function()
    local m = model.new()
    m = model.set_cell_state(m, 1, "running")
    m = model.set_cell_state(m, 1, "success", "val x = 42")
    local cell = model.get_cell_state(m, 1)
    assert.is_nil(cell.duration_ms)
  end)

  it("duration_ms is nil when metadata has no duration_ms", function()
    local m = model.new()
    m = model.set_cell_state(m, 1, "running")
    m = model.set_cell_state(m, 1, "error", "type error", {})
    local cell = model.get_cell_state(m, 1)
    assert.is_nil(cell.duration_ms)
  end)

  it("duration_ms survives running → success transition", function()
    local m = model.new()
    m = model.set_cell_state(m, 1, "running")
    m = model.set_cell_state(m, 1, "success", "ok", { duration_ms = 42 })
    -- re-run
    m = model.set_cell_state(m, 1, "running")
    local cell = model.get_cell_state(m, 1)
    assert.is_nil(cell.duration_ms)
    -- new result with different duration
    m = model.set_cell_state(m, 1, "success", "ok2", { duration_ms = 99 })
    cell = model.get_cell_state(m, 1)
    assert.are.equal(99, cell.duration_ms)
  end)

  it("stale preserves duration_ms", function()
    local m = model.new()
    m = model.set_cell_state(m, 1, "running")
    m = model.set_cell_state(m, 1, "success", "ok", { duration_ms = 200 })
    m = model.mark_all_stale(m)
    local cell = model.get_cell_state(m, 1)
    assert.are.equal("stale", cell.status)
    assert.are.equal(200, cell.duration_ms)
  end)
end)
