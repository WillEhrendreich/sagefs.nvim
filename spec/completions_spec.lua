...
describe("completions", function()
  ...
  describe("build_request_body", function()
    it("returns the MCP JSON completion request contract", function()
      local body = completions.build_request_body("let x = 1", 5, "/work/project")
      assert.are.equal("let x = 1", body.code)
      assert.are.equal(5, body.cursor_position)
      assert.are.equal("/work/project", body.working_directory)
    end)
  end)
