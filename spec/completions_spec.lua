...
describe("completions", function()
  ...
  describe("build_request_body", function()
    it("returns table with code, cursorPos, sessionId", function()
      local body = completions.build_request_body("let x = 1", 5, "my-session")
      assert.are.equal("let x = 1", body.code)
      assert.are.equal(5, body.cursorPos)
      assert.are.equal("my-session", body.sessionId)
    end)
  end)
